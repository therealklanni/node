...
const createDynamicModule = (exports/*: string[]*/, url = '', evaluate/*: function? */) => {
  debug(`creating ESM facade for ${url} with exports: ${ArrayJoin(exports, ', ')}`);
  const names = ArrayMap(exports, name => `${name}`);
  // sanitized ESM for reflection purposes
  let src = `export let executor;
...
// Strategy for loading a node builtin CommonJS module that isn't through normal resolution
...
const normalizeBaseURL = (baseURLOrString) => {
  ...
  return new URL(baseURLOrString);
}
...
const resolveRequestUrl = (baseURLOrString, specifier) => {
  ...
  if (url.protocol !== 'file:') {
    throw Error('Module must be on file: protocol');
  }
