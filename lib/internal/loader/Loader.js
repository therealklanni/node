...
class Loader {
  ...
  async resolve(specifier) {
    let request = resolveRequestUrl(this.base, specifier);
    if (request.url.protocol !== 'file:') {
      throw Error(`Expected to resolve a file: url`);
    }
    return request.url;
  }
  ...
  async getModuleJob(dependentJob, specifier) {
    if (!this.moduleMap.has(dependentJob.url)) {
      throw Error(`unknown dependent ModuleJob ${dependentJob.url}`);
    }
    let request = await resolveRequestUrl(dependentJob.url, specifier);
  ...
  async import(specifier) {
    let request = await resolveRequestUrl(this.base, specifier);
    const url = `${request.url}`;
    let job;
    if (this.moduleMap.has(url)) {
